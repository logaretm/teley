<template>
  <div class="flex-1 flex overflow-hidden bg-zinc-950">
    ...
    <aside
      class="bg-zinc-950 border-r border-zinc-800 flex flex-col overflow-hidden shrink-0"
      :style="{ width: metricsPanelWidth + 'px' }"
    >
      <!-- Header -->
      <div class="border-b border-zinc-800 px-4 py-3">
        <div class="flex justify-between items-center">
          <h2
            class="text-lg font-semibold text-zinc-100 inline-flex items-baseline gap-1.5"
          >
            Metrics
            <button
          ...
          <span class="text-xs text-zinc-500"
            >{{ groupedMetrics.length }} metric{{
              groupedMetrics.length === 1 ? '' : 's'
            }}</span
          >
        </div>
      ...
      >
        <div v-if="loading" class="flex items-center justify-center h-32">
          <div
            class="w-6 h-6 border-2 border-zinc-700 border-t-blue-500 rounded-full animate-spin"
          ></div>
        </div>

        <div
          v-else-if="filteredGroupedMetrics.length === 0"
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      <template v-if="selectedGroup">
          ...
          <div class="flex items-center justify-between">
            <div>
              ...
              <p
                ...
                class="text-sm text-zinc-400 mt-0.5"
              >
        ...
        <div class="flex-1 p-6 overflow-auto">
          ...
          <!-- Data points table -->
          <div
            class="mt-4 bg-zinc-900 border border-zinc-800 rounded-lg overflow-hidden"
          >
              ...
              <table class="w-full text-sm">
                <thead class="sticky top-0 bg-zinc-900">
                  <tr class="border-b border-zinc-800">
                    <th
                      class="text-left px-4 py-2 text-xs font-medium text-zinc-500"
                    >
                      Time
                    </th>
                    <th
                ...
                <tbody>
                  ...
                  >
                    <td
                      class="px-4 py-1.5 text-zinc-400 tabular-nums whitespace-nowrap"
                    >
                      {{ formatTimestamp(dp.timestamp) }}
                    </td>
                    <td class="px-4 py-1.5 text-zinc-100 tabular-nums">
                      <template v-if="dp.type === 'histogram' && dp.histogram">
                        count={{ dp.histogram.count }}, sum={{
                          formatNumber(dp.histogram.sum)
                        }}, min={{ formatNumber(dp.histogram.min) }}, max={{
                          formatNumber(dp.histogram.max)
                        }}
                      </template>
                    ...
                    <td class="px-4 py-1.5 text-zinc-400">
                      {{ dp.service_name }}
                    </td>
                    <td
                      class="px-4 py-1.5 text-zinc-500 text-xs max-w-xs truncate"
                    >
                      {{
                        Object.keys(dp.attributes).length
                          ? JSON.stringify(dp.attributes)
                          : '-'
                      }}
                    </td>
                  </tr>
    ...
    <!-- Setup Guide Modal -->
    <ModalDialog
      ref="setupGuideDialog"
      title="Metrics Setup Guide"
      size="large"
    >
      <MetricsSetupGuide />
    </ModalDialog>
...
const {
  ...
  dragging: metricsPanelDragging,
  onMouseDownLeft: onMetricsPanelMouseDown,
} = useResizablePanel('metrics-panel-width', 340);
...
const selectedGroup = computed(() => {
  ...
  return (
    groupedMetrics.value.find((g) => g.name === selectedMetricName.value) ??
    null
  );
});
